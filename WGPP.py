...
class WGPP(Birthday):
    def alert_user(self,dob,today,months):
    ...
    def validity_check(self,dob,today):
        dob=dob.replace(year = today.year)
        result = 'none'
        #month_diff = relativedelta.relativedelta(dob,today).months
        month_diff = abs(dob.year-today.year)*12 + (dob.month - today.month)

        if today < dob and month_diff<=2:
                month_diff_new =(dob.year-today.year)*12+(dob.month-today.month)

                days_to_renewal, weeks_to_renewal = Birthday.days_count(self,dob, today)
                result= Birthday.upcoming_birthday(self,days_to_renewal, weeks_to_renewal,dob, today,month_diff_new )
                return result
            ...
            days_to_renewal, weeks_to_renewal = Birthday.days_count(self,dob, today)
            result = Birthday.passed_birthday(self,days_to_renewal, weeks_to_renewal,dob, today )
            return result
            ...
            dob_2 = dob.replace(year=today.year + 1)
            dob = dob_2
            days_to_renewal, weeks_to_renewal = Birthday.days_count(self,dob_2,today)
            month_diff_new =(dob.year-today.year)*12+(dob.month-today.month)
            ...
            if month_diff_new <=2:
                ...
                result= self.alert_user(dob,today,month_diff_new)
                return result
                print("Your WGPP Annual Membership due date is {} on 15-Mar-2021".format(result))
